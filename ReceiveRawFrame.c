...
#define BUFFER_SIZE 1526 // This indicates maximum buffer size for the whole ethernet frame.
...
void send_packet(const char *interface, const uint8_t *buffer, ssize_t length) {
    ...
    udp->check = /*0;*/ checksum_udp(check_buffer,ucheck_size);
    
    
    /*printf("------------------------------ \n");
    for (int i = 0; i < ucheck_size; i++)
        printf("%02x:", (unsigned char) check_buffer[i]);
    printf("\n");
    printf("------------------------------ \n");*/
